...
class SqlDelightMeshDao(
    ...
    override suspend fun insertMessage(message: MeshMessage) = withContext(Dispatchers.IO) {
    ...
    override suspend fun getAllMessages(): List<MeshMessage> = withContext(Dispatchers.IO) {
    ...
    override suspend fun getPendingSyncMessages(): List<MeshMessage> = withContext(Dispatchers.IO) {
        db.meshQueries.selectPendingSync().executeAsList().map {
            MeshMessage(it.id, it.senderId, it.recipientId, it.chatId, it.content, it.timestamp, it.type, it.isSynced ?: false)
        }
    }

    override suspend fun markAsSynced(id: String) = withContext(Dispatchers.IO) {
        db.meshQueries.markAsSynced(id)
    }

    override suspend fun insertPeer(peer: MeshPeer) = withContext(Dispatchers.IO) {
        db.meshQueries.insertPeer(peer.id, peer.name, peer.lastSeen)
    }

    override suspend fun getAllPeers(): List<MeshPeer> = withContext(Dispatchers.IO) {
